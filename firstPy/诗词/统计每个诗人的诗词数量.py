...
def process():
    what = ['id']
    i =0
    for each in list(db._select2dic(tablename='shiren', what=what,)):
        shiren_id = each['id']
        #shiren_id =18750
        print shiren_id
        try:
            for count, in db._execute("SELECT count(1) FROM shici where author_id=%s" % shiren_id):
                print count
            try:
                where = 'id=%s' % shiren_id
                update_dict = {}
                update_dict['num'] =count
                update_db._update(where=where, **update_dict)
            except:
                traceback.print_exc()
        except:
            traceback.print_exc()
