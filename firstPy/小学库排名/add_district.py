...
where = '`city` = %s and district = "重庆"' % db.placeholder
update_where = '`name` = %s' % db.placeholder
for each in db._select2dic(where=where, what=['name','address'], where_values=(u'重庆',)):
    try:
        #print  each['name']
        old_name = each['name']
        old_address = each['address']
        name = old_name.replace(u'重庆','').replace(u'重庆','')
        address = old_address.replace(u'重庆','')
        add_part = ''
        if re.match(ur'.*?区', name):
            add_part = re.match(ur'.*?区', name).group(0)
            add_part = re.sub(ur'.*?市', '', add_part)
        elif add_part=='':
            if re.match(ur'.*?区', address):
                add_part = re.match(ur'.*?区', address).group(0)
            add_part = re.sub(ur'.*?市', '', add_part)
        if add_part=='':
            pass
        else:
            update_dict = {}
            update_dict['district'] = add_part
            print old_name,add_part
            print update_db._update(where=update_where, where_values=(old_name,), **update_dict)
    except:
        traceback.print_exc()
        continue
