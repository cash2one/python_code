...
import sys,json,oss2,hashlib,traceback,os,MySQLdb
reload(sys)
...
for file in os.listdir('/Users/bjhl/Documents/imgs_add'):
    name = file.split('.')[0].strip()
    cursor.execute("select id, display_image from primary_school_info where name='%s'" % name)
    for (id, display_image) in cursor.fetchall():
        if 'bdimg' in display_image:
            new_display_image = get_image('/Users/bjhl/Documents/imgs_add/'+file)
            print new_display_image
            sql = "update primary_school_info set display_image = '%s' where `id` = %d" % (new_display_image,id)
            cursor.execute(sql)
            conn.commit()