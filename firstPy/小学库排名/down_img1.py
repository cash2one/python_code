...
count = 0
fw = open('/Users/bjhl/Documents/error_img','a')
for each in update_db._select2dic(what=['id','display_image']):

    try:
        id = each['id']
        display_image = each['display_image']
        filename = '/Users/bjhl/Documents/imgs_edit/' + str(id) + '.jpg'
        if 'zhanqun' not in display_image:
            ...
            resp = response.read()
            print id, display_image

            with open(filename,'wb') as f:
                f.write(resp)
                f.flush()
        # elif 'zhanqun' not in display_image:
        #     count += 1
        #     urllib.urlretrieve(display_image,filename)
        #     print id, display_image

    except:
        fw.write(str(id)+'\t'+display_image+'\n')
        fw.flush()
        traceback.print_exc()
        continue
print count
